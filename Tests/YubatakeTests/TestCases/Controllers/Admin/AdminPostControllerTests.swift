...
final class AdminPostControllerTests: ControllerTestCase {
    ...
    func testCanViewPageButtonAtTwoPages() throws {
        ...
        try test(.GET, "/admin/posts?page=2", afterResponse:  { response in
            XCTAssertEqual(response.status, .ok)
            XCTAssertEqual(view.get("metadata.total")?.int, 11)
    ...
    func testCanViewStaticContent() throws {
        ...
        try post.save(on: db).wait()
        try test(.GET, "/admin/static-contents", afterResponse:  { response in
            XCTAssertEqual(response.status, .ok)
            XCTAssertEqual(view.get("items")?.array?.count, 1)
            XCTAssertEqual(view.get("metadata.total")?.int, 1)
            XCTAssertEqual(view.get("metadata.page")?.int, 1)
    ...
    func testCanViewCreateView() throws {
        try test(.GET, "/admin/posts/create", afterResponse:  { response in
            XCTAssertEqual(response.status, .ok)
        })
    }
    
    func testCanViewEditView() throws {
        try test(.GET, "/admin/posts/1/edit", afterResponse:  { response in
    ...
    func testCanDestroyPosts() throws {
        let post = DataMaker.makePost(title: "title", content: "content", htmlContent: "htmlContent", partOfContent: "partOfContent", userId: 1)
        try post.save(on: db).wait()
        try test(.POST, "/admin/posts/delete", body: "posts[]=1", withCSRFToken: false, afterResponse:  { response in
            XCTAssertEqual(response.status, .forbidden)
        })
        try test(.POST, "/admin/posts/delete", body: "posts[]=1", afterResponse:  { response in
    ...
    func testCanStoreAPost() throws {
        ...
        try test(.POST, "/admin/posts", body: form, withCSRFToken: false, afterResponse:  { response in
            XCTAssertEqual(response.status, .forbidden)
        })
        try test(.POST, "/admin/posts", body: form, afterResponse:  { response in
            XCTAssertEqual(response.status, .seeOther)
    ...
    func testCanUpdateAPost() throws {
        ...
        try test(.POST, "/admin/posts/1/edit", body: form, withCSRFToken: false, afterResponse:  { response in
            XCTAssertEqual(response.status, .forbidden)
        })
        try test(.POST, "/admin/posts/1/edit", body: form, afterResponse:  { response in
            XCTAssertEqual(response.status, .seeOther)
            XCTAssertEqual(response.headers.first(name: .location), "/admin/posts/1/edit")
        })
        try test(.GET, "/admin/posts/1/edit", afterResponse:  { response in
